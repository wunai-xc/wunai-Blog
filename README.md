...
- 首页：三屏**吸附式**（scroll-snap）——首屏问候 / 更新内容（最近 5 次提交）/ 随便看看（交错卡片 + 全部文章入口），右侧分页指示点可点击跳屏；用 `proximity` 温和吸附，并由 `HomeSnap` 做「向前拉 / 向后并」两个辅助吸附（**不用 `mandatory`**，否则会把后面的页脚封住）
...
│   │   ├── HomeSnap.tsx            # 首页吸附加力（两个辅助吸附；刻意不用 mandatory）
