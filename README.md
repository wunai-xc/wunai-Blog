...
- 首页：三屏**吸附式**（scroll-snap）——首屏问候 / 更新内容（最近 5 次提交）/ 随便看看（交错卡片 + 全部文章入口），右侧分页指示点可点击跳屏；用 `proximity`（不用 `mandatory`，否则会把后面的页脚封住），加力由 `HomeSnap` 的「向前拉 / 向后并」两个辅助吸附提供；首页页脚额外设 `scroll-snap-align: end`，保证能滚到文档最底
...
- 角标：仅在**面板**内侧 5px 处画矩形框 + 左上/右下两枚十字（小元件、按钮、标签一律不加）。线条为“交点色 → 终点色”渐变，全图只有两个交点是满色：**两个主题的交点色都跟随主色 `--accent`，终点色取页面底色**（所以读起来是从交点淡出，而非一条实线）。线宽 `--cn-w` 为 0.5px（高分屏上半像素发丝）。覆盖文章卡片、上下篇、友链卡片、最近更新面板、目录面板、设置面板。`<input>`（.search-box）因伪元素不渲染、代码块与表格因自身横向滚动而排除。**`.post-content`（正文阅读面）刻意不参与**：框的长度是 `100% - inset × 2`，在万像素高的文章上会成为四条贯穿全文的线（“四角相连”在文档级高的面板上不成立），而且满尺寸的 `::after` 会独立成层、与 `backdrop-filter` 一起吃掉几十 MB 显存。
...
│   │   ├── HomeSnap.tsx            # 首页吸附加力（向前拉 / 向后并；刻意不用 mandatory）
