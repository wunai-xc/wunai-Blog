...
| 2. 最近更新 | 最近 5 次提交（日期 + 信息 + 短 sha，可点进 GitHub），数据来自 `public/changelog.json` |
| 3. 展示位 | 最多 3 篇卡片 |
...
4. **向下滑动图标**：锚点默认 `#home-updates`（最近更新区块），无更新数据时回退 `#home-posts`；复用 `html { scroll-behavior: smooth }`。在简介版首屏绝对定位钉底，在关于版里跟在正文后正常排版。
5. **展示位**：`getHomeShowcase()` 取最多 3 篇（有置顶取置顶，否则取最新非 AI），并用 `ScrollReveal` 包裹实现滚动到位渐入；文章卡片传 `hidePinnedBadge` 不显示“置顶”徽标。
6. **最近更新**：`getChangelog()` 读 `public/changelog.json`；文件缺失或损坏时返回空数组，该区块自动不渲染，向下滑动图标的锚点也随之回退到 `#home-posts`，不会指到空锚点。
...
| 元素 |
|------|
| `.post-card`、`.post-nav a`、`.friend-card`、`.changelog-list`、`.collection-card`、`.toc-panel`、`.settings-panel` |
...
- 参与：`.home-hero h1`、`.home-about-title`、`.page-title`、`.archive-year`、`.not-found h1`
- 不参与：`.post-header h1`（文章页大标题）、`.changelog-head`（原因见下两条）
...
- **带渐入动画的容器内的标题不能参与**：`.reveal` 在动画期间有 `transform` 与 `opacity < 1`，同样是 backdrop root，标题会先以白色淡入、动画结束才突变成黑。`.changelog-head` 就是因此不参与。若以后要把某个标题放进 `ScrollReveal`，记得同步从名单里移除。
...
所以本站的做法是：**太宽的公式自动等比缩小，直到刚好放得下**。

```
装得下          → 原尺寸居中显示
太长            → 自动缩字号（下限 0.6，即最小缩到原六成）
缩到下限还是不行 → 保持原样，可横向拖看（极少见）
```

所以写公式时**不需要**为了适配而手工拆行或改小，直接写完整式即可。
若某条公式缩得太狠、看不清，再考虑改成多行（用 `\begin{aligned}` 或 `\begin{split}`）。
...
| `ok` | 本来就装得下，没动 |
| `fit:0.82` | 缩到 82% 后放下了 |
| `skip:0.41` | 需要缩到 41%，低于下限 0.6，不缩、改为可横向拖 |
| `failed` | 迭代后仍放不下，已恢复原样 |
