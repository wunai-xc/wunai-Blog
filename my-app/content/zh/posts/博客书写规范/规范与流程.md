...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.13 | 2026-09-19 | 改正 1.12 的放置位置：搜索 / 标签浮动按钮从**文章详情页**移到**文章列表页** `/<lang>/posts/`（详情页只留目录 + 正文字号）。因此格位改用 `--fab-slot` + `--fab-shift` 两个变量计算（两页各一列，写死的格号必然对不上）；2.4 节重写 |
| 1.12 | 2026-09-19 | 文章页左边缘浮动按钮列扩到四个（目录 → 搜索 → 标签 → 正文字号）：新增 `components/ArticleQuickLinks.tsx`（搜索/标签是链接不是面板）与标签总览页 `/zh/tags/`（此前只有单个标签页）；列位改用 `.post-layout:has(.toc-fab)` 统一移位，替掉原先的相邻兄弟选择器 |
| 1.11 | 2026-09-19 | 文章页新增「正文字号」浮动按钮（目录按钮下面一格，5–32px）：新增 `components/ArticleFontSize.tsx`、`--article-fs` / `--article-fs-pref` / `--fab-step`，正文排版字号全部改为 `--article-fs` 的倍数（原先写死 `rem`，字号一改就会与正文脱节）；新增 2.4 节说明「字号只有一个来源」的约定 |
| 1.10 | 2026-09-19 | 新增「关于」独立页面（`/zh/about/`、`/en/about/`）：正文写 `content/<lang>/about.md`，由 `getPage()` 读取；顶栏 `About……` 改为指向该页面；`renderMarkdown` 参数放宽为最小形状 `MarkdownSource`（原先写死 `Post`），并新增 4.5 节说明独立页面与「关于」文章的区别 |
| 1.9 | 2026-09-19 | 修桌面端「滚不到底」：① 去掉 `.home-page` 的 `scroll-snap-stop: always`（它禁止滚动跨过吸附点，把最后一屏起点变成挡着页脚的墙）；② 给首页页脚加 `scroll-snap-align: end`，使「文档最底部」本身成为合法停靠位置（原先最大滚动量离最后一屏起点不到一屏，proximity 会把页脚反复拉回去） |
| 1.8 | 2026-09-19 | 修正 1.7 的错误：首页吸附**不能用 mandatory** -- 三屏之后还跟着页脚，mandatory 会把页脚封在外面（实测滚不到页脚）。改为只加强 proximity：HomeSnap 提供「向前拉」与「向后并」两个辅助吸附，并在靠近文档底部时完全不动手 |
| 1.7 | 2026-09-19 | 首页吸附加强：新增 HomeSnap，三屏都装得下时升到 mandatory，否则保持 proximity 并做「只向前」的辅助吸附 |
| 1.6 | 2026-09-19 | 背景点网格加密（间距 26/22 → 20/17，点数上限 3000 → 4200）并调淡静止点（档 0 透明度 0.3 → 0.18） |
| 1.5 | 2026-09-19 | 新增 ReadingHeader（阅读时顶栏自动隐藏、双击呼出）与 `--chrome-top`：修正粘性标题 / 目录按钮写死 56px 而被手机上 70px 顶栏遮盖的问题；入场动画改为卡片整块滑入（原先只动卡片内的文字）、填充模式 both → backwards（顺带修好 hover 位移被动画压过的一批失效）；角标暗色改为与亮色同一套读法，线宽 1px → 0.5px |
| 1.4 | 2026-09-19 | 新增配套的《博客维护指南》（也是卡组）并在文首建立互链；修正卡组小节里关于搜索/RSS 的说法（两个构建脚本原本不递归子目录，卡组内文章会从搜索与 RSS 掉出去，已修）；补提醒：改内容目录结构需同步改三处 |
...
**想看「这件事具体怎么做」而不是「为什么要这样做」**，去配套的 [《博客维护指南》](/zh/groups/维护指南/)：
那是任务导向的操作手册，本文侧重约定与原理。两者需同步维护。
...
10. **首页 = 三屏吸附**：首屏问候 / 更新内容（最近 5 次提交）/ 随便看看（交错卡片 + 全部文章入口），右侧分页指示点可点击跳屏；详见 3.1。
11. **设置页**：配色方案（预设 + 取色器）、阅读宽度、背景动效、亚克力开关、动画强度，全部只存本机。
12. **友链页 + 页脚联系方式**：友链卡片列表（图片/名称/介绍），页脚展示邮箱、GitHub、仓库、哔哩哔哩、YouTube、微信、Discord（图标为本地打包，国内可正常显示），以及设置与语言切换入口。
13. **浮动导航**：文章列表页左边缘的搜索 / 标签按钮，文章详情页的目录 + 面板与正文字号按钮（5–32px），右侧可拖拽的阅读进度滑块（章节圆点）、回到顶部，均不占布局宽度。
14. **阅读时的顶栏**：文章页滚过 120px 后顶栏自动上移隐藏（把整屏留给正文），双击（鼠标或触屏）缓缓滑出；回到页面顶部自动恢复「跟随滚动」。粘性标题 / 目录按钮会跟着上移占位，见 `components/ReadingHeader.tsx`。
15. **卡组（文章组）**：同主题多篇折叠为一张叠层卡片，见 5.4。
...
│       ├── about/
│       │   └── page.tsx         # 「关于」独立页面：渲染 content/<lang>/about.md（改文案改 md，不改这里）
│       ├── archives/
...
│               └── page.tsx     # 单个标签下的文章列表
│
├── components/                  # React 组件（共 22 个）
│   ├── Header.tsx               # 顶栏：品牌区（主题切换 + 大号 logo + 小字行）+ 友链 + 图片区
│   ├── HeaderIntro.tsx          # 顶栏入场：级联淡入（渐进增强）+ 光标切后台暂停
│   ├── ReadingHeader.tsx        # 阅读时的顶栏行为：滚过阈值自动隐藏，双击缓缓滑出
│   ├── Footer.tsx               # 页脚：欢迎语 + 联系方式/社交/设置/语言卡片 + 「感谢你的阅读」 + 版权行
│   ├── PostCard.tsx             # 文章卡片：静态亚克力材质 + 边线 hover + AI 配色变体
│   ├── GroupCard.tsx            # 卡组卡片：静态错位线框 + 封面填满边框
│   ├── PageIndicator.tsx        # 首页右侧分页指示点（高亮当前屏 + 点击跳页）
│   ├── HomeSnap.tsx             # 首页吸附加力：三屏都放得下则升到 mandatory，否则做向前辅助吸附
...
│   ├── ArticleQuickLinks.tsx    # 列表页左边缘浮动按钮：搜索 / 标签（链到 /<lang>/search/ 与 /<lang>/tags/）
│   ├── ArticleFontSize.tsx      # 正文字号浮动按钮：5–32px，只改正文（见 2.5）
...
│   ├── FontSizeControl.tsx      # 旧的全站字号档位（改 <html> 根字号，85%–130%）；已无任何页面挂载，保留作参考，文章页请用 ArticleFontSize.tsx
...
│ 注：`content/<lang>/about.md`（上方）是「关于」**页面**的正文，路由 `/zh/about/`；
│     与这里 `about: true` 的「关于」**文章**是两回事：文章是普通文章（首页三屏已不渲染它的正文，
│     `about: true` 现在只让它不进首页展示位），页面是独立 URL。
│
...
- 取数辅助函数：`getPosts` / `getPost` / `getPinnedPosts` / `getPrevNext` / `getArchives` / `getAllTags` / `getPostsByTag` 等；首页相关两个：
  ...
  - `getAboutPost(lang)` —— 取 `about: true` 的文章。**目前没有调用方**（首页三屏已不渲染关于正文，
  顶栏的「About……」改指独立页面）；保留是因为它仍是合法取数，`about: true` 也确实还在用
  —— 作用是让那篇文章不进首页展示位。
- **独立页面**：`getPage(lang, name)` —— 读 `content/<lang>/<name>.md`（如 `about.md`），返回 `{ lang, name, title, description?, content, references? }`；文件不存在时返回 `undefined`，由页面自己 `notFound()`。它走的是**另一条路**：不参与上面任何文章查询，所以天然不进列表 / 归档 / 标签 / 搜索索引 / RSS。
...
`renderMarkdown(src)` 返回 HTML 字符串。参数是**最小形状** `MarkdownSource`（`lang` / `slug` / `content` / `references?`）：文章页直接传整个 `Post`（结构上满足它），独立页面（如「关于」）传 `getPage()` 的结果就行 —— 不必为了渲染一段 Markdown 去凑 `tags`、`wordCount` 这些与它无关的字段。管道顺序（**顺序很重要，不能乱**）：
...
13. `rehypeImages(src)` — **自定义插件**：图片懒加载、相对路径转绝对
...
- **档 0 的透明度（`BAND_ALPHA[0]`）就是全部静止点的亮度**，调背景点深浅改它即可。
  当前为 0.18（浅色主题；暗色再乘 `alphaScale`）。原来 0.3 偏显眼，与内容争视觉焦点。
- **间距与点数上限必须联动**：`buildGrid()` 会用 `sqrt(w×h / MAX_DOTS)` 算出最小间距，
  超出就自动放大。所以调小 `BASE_SPACING` 时若不抬高 `MAX_DOTS`，
  大屏会被自动改回稀疏的间距 —— 调间距等于白调。当前 `20 / 17px`，`MAX_DOTS` 4200。
...
1. **吸附写在 `<html>` 上，不另建 `height:100vh` 的内部滚动容器。**
   ...
   那样会把页脚永久挡住、读者根本够不到。
2. **强度只能用 `proximity`。** 加力靠 `components/HomeSnap.tsx` 的两个辅助吸附，
   **不能用 `mandatory`** —— 后者要求滚动容器只能停在吸附点上，
   而首页三屏之后还跟着页脚、页脚**没有吸附点**，于是滚到第 3 屏起点后再往下就会被弹回，
   **页脚永远够不到**。这跟屏高无关：只要最后一屏后面还有内容，mandatory 就会封住它。
   （曾按「三屏是否都装得下视口」判定后升级到 mandatory，正是漏了这一条：
   卡片不足 6 张时第 3 屏会变矮、通过判定，页脚随即被封死。）

   HomeSnap 的两个辅助吸附（都是「停稳后帮一把」，不改变「滚动可停在任意位置」这个前提）：

   | 辅助 | 触发条件 |
   |------|----------|
   | 向前拉 | 停稳时前方某屏的起点在半屏以内，且那一屏自己放得下 |
   | 向后并 | 停稳时已略微越过某屏起点（在 14% 屏高以内） |

   另有一条守卫：**靠近文档底部时完全不动手**，从机制上保证页脚可达。
3. **用选择器限定作用域**：`html:has(.home-pager)`。
   不限定的话，`scroll-padding-top` 会连带影响文章页的锚点跳转——
   那里已有 `.article h2 { scroll-margin-top: 80px }`，两者会叠成双重偏移。
4. **不要写 `scroll-snap-stop: always`。** 它的语义是「滚动不得跨过吸附点」，
   于是最后一屏的起点成了一道墙，想再往下到页脚会被拦在那里。
   手机上第 3 屏超过一屏、离页脚远，还不明显；桌面上三屏刚好一屏高、
   页脚紧跟其后，这个拦截就表现为「滚不到底」。`proximity` 本身已够温和。
5. **页脚也要参与吸附（`align: end`，仅限首页）。** 首页三屏都是「刚好一屏高」、
   页脚又比一屏矮，于是最大滚动量只落在最后一屏起点之后**不到一屏**的位置；
   而 `proximity` 会在停止点离吸附点足够近时把它拉回去 ——
   桌面上这个距离正好落进范围内，页脚因此被反复拉回第 3 屏。
   给页脚加一个 `end` 对齐的吸附点后，「文档最底部」成了合法停靠位置，
   而且 end 对齐算出的位置恰好等于最大滚动量（不需要超出范围再被截断）。
6. **每屏高度是 `calc(100svh - var(--header-h))`**，不是 `100vh`：
   顶栏 sticky 常驻，要扣掉它；用 `svh` 而非 `vh` 是为了避开手机地址栏伸缩引起的跳动。
7. **高度用 `min-height`**：更新列表较长时允许本屏自然变高，不会被固定高度裁掉内容。
8. **`scroll-padding-top: var(--header-h)`** 让吸附位置落在顶栏下方，而不是被顶栏压住。
9. **向下提示的关键帧要带 `translateX(-50%)`**，否则动画的 `transform` 会把居中位移覆盖掉。
10. **打印时关掉吸附**（`scroll-snap-type: none`），并把交错网格改回单列顺序排列，
    否则纸上会左右错位失衡。
...
- **`About……` 指向「关于」独立页面 `/<lang>/about/`**（正文在 `content/<lang>/about.md`，由 `getPage` 读），
  不再是 `about: true` 的那篇文章；页面文件缺失时回退到文章列表，避免死链。
- **`--header-h` 是顶栏高度的唯一来源**：`.navbar` 直接用它做固定高度，
  因此该变量与实际高度必然一致。首屏高度、锚点偏移都拿它定位，
  断点处只改这一个值（平板 78px / 手机 70px）。
  
  ⚠️ **阅读区的浮动控件不能用 `--header-h`，要用 `--chrome-top`**（初值 = `--header-h`）。
  因为顶栏在阅读时会隐藏，那时浮动控件需要归零上移；
  早先粘性标题 / 目录按钮写死 `56px` / `57px` / `96px`，
  而手机上顶栏是 70px —— **顶栏比它高，直接把它们盖住了**。
  改顶栏高度时请同时确认 `--chrome-top` 仍是 `var(--header-h)`。
  左侧那一列浮动按钮（目录 / 搜索 / 标签 / 正文字号）除了 `--chrome-top`
  还要加 `N × --fab-step` 取列位，详见 2.4。
...
#### `components/ReadingHeader.tsx` — 阅读时的顶栏行为

**只在文章页渲染。** 滚过 120px 后自动隐藏顶栏，双击（鼠标或触屏）缓缓滑出。

两个设计决定，改动前先看这里：

1. **滚离顶部才隐藏。** 顶栏是 `sticky`，它的占位仍在文档最顶上；
   若在 `scrollY = 0` 就上移，页面顶部会露出一条与顶栏等高的空白带。
   滚过阈值后那块占位已在视口之上，上移只会揭出被它盖住的内容。
2. **双击切的是「临时呼出」，不是「永久显示」。** 双击后保持展开，
   回滚到顶部时自动复位，回到「跟随滚动」的常态 —— 不会因一次误触把「阅读时隐藏」永久关掉。

实现方式：往 `<html>` 上写 `data-header="hidden" | "shown"`，样式在 `globals.css`；
组件卸载时移除该属性，离开文章页顶栏立刻恢复正常。
隐藏用 `transform: translateY(-100%)` 而不是 `height`（后者会让 sticky 元素每帧重排整篇文档）。

双击识别同时监听 `touchend` 与 `dblclick`，并用 400ms 去重窗口吞掉同一次手势多路触发。
...
卡片、面板、按钮、图片和标签使用 `border-radius: 0`；首页分页点也改为方形。首页采用低对比度网格与页内水平分隔线，文章卡片 hover 使用左侧内线与轻微位移，避免 3D 跟随与弹跳。进度控件中仍有独立的圆形装饰，不要为了直角规范破坏进度语义。
...
- **亚克力材质与悬停动效分离**：卡片的材质层仍在 `::after` 中；卡片自身不再写入 JS 3D 变换，悬停只做轴向轻移、边框变色与左侧 2px 内线。正文阅读面入场仅淡入/平移，不缩放长文，避免文字发虚。
...
|------|------|------|------|
| 亮色 | `var(--accent)` 蓝 | `#ffffff` | 蓝 → 白（白即卡片底色，看起来就是淡出） |
| 暗色 | `var(--accent)`（暗色主色会亮一档） | `var(--bg)` | 同亮色一套读法，只是底色不同 |
...
**颜色 token**：两个都定义在 `:root` 并在 `html.dark` 里覆盖 —— `--cn-color`（交点色）两个主题都跟随 `--accent`，`--cn-end`（终点色）亮色 `#ffffff` / 暗色 `var(--bg)`。伪元素规则里只引用变量，所以换配色只改这两个 token。
...
| 元素 |
|------|
| `.post-card`、`.post-nav a`、`.friend-card`、`.update-item`、`.home-card`、`.collection-card`、`.toc-panel`、`.font-panel`、`.settings-panel` |
...
- **小元件**（`.pagination`、`.term-item`、`.tag`、`.author-badge`、`.print-btn`、`.footer-link`、`.back-to-top`、`.toc-fab`、`.search-fab`、`.tags-fab`、`.font-fab`）：它们数量多、叠上去显得吵；而且 20px 高的标签本就塞不下 5px 内缩的框，需要另调一套数值。已明确不要。
...
- **颜色** `--cn-color`（交点）：两个主题都跟随 `--accent`；`--cn-end`（终点）：亮色 `#ffffff` 白 / 暗色取 `var(--bg)`。
  两者都在 `html.dark` 里覆盖，没有写进伪元素规则，所以换配色只改这两个 token。
  
  ⚠️ **暗色曾经是「暗红 → 黑」（`#7f1d1d` / `#000000`），与亮色的「蓝 → 白」差距很大，
  两个主题看起来像两套设计。**现已改成与亮色同一套读法：交点色跟 `--accent`、
  终点色取页面底色，于是两个主题只是底色不同，线的读法一致。
...
调参：`--cn-color`（交点色）/ `--cn-end`（终点色）/ `--cn-inset`（内缩）/ `--cn-arm`（臂长）/ `--cn-w`（线宽，现为 **0.5px**，高分屏上按半像素渲染），整体浓度改伪元素上的 `opacity`（默认 0.6）。
...
**2.3 入场动画：文字逐个来，卡片整块来**

全站共用一段关键帧 `slide-in-from-right`（从右 30px + 缩 0.95 滑入，中途回弹两下），
用 `steps(10, end)` 按刻跳变，时长由 `--enter-duration` 控制。

| 目标 | 选中的元素 | 入场方式 |
|------|-----------|----------|
| 文字 | `.article h1~h4` / `p` / `li` / `blockquote`、归档列表项、标签、分页、上下篇、面包屑、搜索框、页脚 | 逐个滑入，同区块用 `animation-delay` 错开 |
| 卡片 | `.post-card`、`.collection-card`、`.home-card`、`.update-item` | **整块**滑入，框与里面的文字一起来 |

两个容易错的地方：

- **卡片必须动画「卡片本体」，不能只动画它里面的标题/摘要**。
  曾经写的是 `.post-card .title` 等子元素 —— 结果是「文字飞来飞去、卡片框不动」的割裂感；
  而且下面那组 `.post-card:nth-child(n)` 错位延迟因为目标选错而完全没生效。
- **填充模式用 `backwards`，不用 `both`**。
  `both` 会在动画结束后继续占着 `transform`，从而压过 hover 的位移
  （`.tag` / `.term-item` / `.post-nav a` 的浮起，以及 PostCard 用 JS 写的行内 transform）。
  这条动画的 100% 帧与元素常态本就一致，after 阶段不需要保留任何东西，
  所以 `backwards` 在视觉上完全等价。

**尊重系统偏好**：`prefers-reduced-motion: reduce` 与设置页的「动画强度 = 关闭」
都会把动画时长压到 0.01ms。
...
| 页面 | 列 |
|------|-----|
| 文章列表页 `/<lang>/posts/` | 搜索 → 标签（`ArticleQuickLinks`） |
| 文章详情页 `/<lang>/posts/<slug>/` | 目录（`PostNav`，仅当文章有标题）→ 正文字号（`ArticleFontSize`，带面板） |

**为什么搜索/标签放在列表页而不是文章页**：列表页本身就是「文章太多、想快点找到某一篇」的场景；而搜索页与标签云此前在界面上没有任何入口（`SITE.menu` 是死数据，没有任何组件渲染它），这两颗按钮算是把入口接回来。文章页则已经有面包屑与上下篇，且正文里的标签链接本来就存在。

格位算法（**两个变量**，不是写死的格号）：

```css
--fab-slot   本按钮在这列里排第几（从 0 起，每个按钮/面板自己声明）
--fab-shift  整列下移几格（只有目录那列用，.post-layout:has(.toc-fab) 置 1）
top = --chrome-top + 1px + (--fab-slot + --fab-shift) × --fab-step
```

三条硬约定：

1. **不要换回相邻兄弟选择器（`~`）或写死的格号**。前者要求元素串联成链、每加一个按钮都要把后面全部重算；后者在两页各一列的情况下必然对不上。现在是整列一次性移位，各按钮只管自己的 `--fab-slot`。
2. **两个面板（目录、字号）的 `top` 用同一个公式**（`44px` = 按钮高 + 10px 间隙，再接 `(--fab-slot + --fab-shift) × --fab-step`），不能只写固定值 —— 否则面板会飘到与按钮不相邻的位置。
3. `--fab-step` 就是按钮高 + 8px 间隙（`:root` 42px / 手机 38px），**改按钮尺寸时要同步改它**。

再补一句设计取舍：**搜索与标签是链接（`<a>`），不是带面板的按钮**。它们指向已有的 `/<lang>/search/`（自带输入框 + Fuse 全文索引）与 `/<lang>/tags/`（标签云）；在这列里再抄一份搜索框/标签云只会多一份要同步维护的实现。

**2.5 正文排版：字号只有一个来源（`--article-fs`）**

正文字号按钮只改正文，怎么做到“只改正文”以及为什么这样写成了一整套约定：

| 角色 | 是什么 |
|------|--------|
| `--article-fs-pref` | 控件把 px 值写在 `<html>` 的行内样式上（`components/ArticleFontSize.tsx`） |
| `--article-fs` | **只在** `.post-content[data-article-font]` 上定义，值取上面那个（回退 `1rem`） |
| `data-article-font` | 只有文章详情页的 `.post-content` 带这个标记，于是变量的可见范围就只有那一页的正文 |

四条硬约定：

1. **正文内的字号一律写成 `--article-fs` 的倍数**（`calc(1.6 * var(--article-fs, 1rem))`），间距继续用 `em`。
2. **这一段里不要写 `rem`** —— `rem` 认的是 `<html>`，与正文字号无关；混用会出现「正文缩到 5px，标题还停在 25px」的断层。
3. **不要在别处定义 `--article-fs`**。它一旦挂到 `:root` 上，全站都会跟着缩，“只管正文”就没了。
4. **值存 `localStorage('articlefont')`，并在 `app/layout.tsx` 的内联脚本里预置**（首次绘制前就放好，不会先画一遍默认字号再跳）。

为什么分两层变量：内联脚本只能写 `<html>`（<body> 还没解析），而“只作用于文章页”又必须靠页面级的标记 —— 两者分开就同时拿到了「不闪」与「不外溢」。

两个容易忘的联动：

- 公式（KaTeX）内部全用 `em`，会自动跟着缩；但**长公式的缩放比例是 JS 量的**（PostBody），改字号后必须重算 —— 控件为此统一发一个 `resize` 事件（合并到一帧一次），而 PostNav 的目录节点位置/进度条也靠它重算。
- 打印不受影响：`@media print` 里 `.article` 是 `11px !important`，字号设置不进 PDF。

例外与边界：

- 代码块的 Copy 按钮写成 `max(10px, calc(0.72 * var(--article-fs, 1rem)))`：它是控件不是内容，正文缩到 5px 时不该一起缩到点不到。
- 该按钮的位置与列位约定见上方 2.4。
...
| `content/zh/about.md` | `/zh/about/` |

注意 `trailingSlash: true`，URL 末尾带 `/`。

### 4.5 独立页面（「关于」这类固定页）

有些内容不属于「文章」：不进列表、归档、标签、分类、搜索索引和 RSS，也不需要日期。这类内容放在 `content/<lang>/` 下（与 `posts/` 平级），由 `getPage(lang, name)` 读取。
...
**改「关于」页文案 = 只改上面两个 `.md`，页面代码（`app/[lang]/about/page.tsx`）不用动。**

写法与文章正文完全一致（公式、代码高亮、mermaid、`==高亮==`、`[reference:N]` 都可用），只有三点差别：

1. frontmatter 只认 `title`（页面大标题）和 `description`（标题下的一句话）；写了 `date`、`tags` 也不起作用。
2. **正文图片必须用 `/` 开头的绝对路径**（如 `/helloworld/wunailogo.png`）。文章的相对路径会被拼成 `/posts/<slug>/...`，而独立页面没有文章目录，相对路径会指向错的位置。
3. 没有目录、评论、上下篇、打印 / 下载按钮 —— 它就是一个「一页到底」的说明页。

> 入口：顶栏那个「About……」链接就指向这个页面（`components/Header.tsx` 里的 `aboutHref`）。

想再加一个同类页面（比如「许可协议」）：新建 `content/zh/<名字>.md`，再照 `app/[lang]/about/page.tsx` 复制一份路由、把 `"about"` 改成新名字，并在 `app/sitemap.ts` 里加一行。
...
| 上一篇 / 下一篇 | **组内只与组内接续，组外只与组外接续**，两边不互通；组内按 `order` 升序接续（与组外「新的在前」方向相反，实现时要分开处理） |
| 组内文章 URL | 仍是 `/<lang>/posts/<文件名>/`（不带组名），撞名时会自动加组名前缀 |
| 归档 / 标签 / 分类 / 搜索 / RSS | 组内文章照常出现（它们仍是普通文章） |

> ⚠️ **搜索与 RSS 依赖构建脚本自己递归子目录。** 它们不经过 `lib/content.ts`，
> 而是各自用 `readdirSync` 扫 `content/<lang>/posts/`。
> 早期版本只读顶层 `.md`，导致卡组内文章从搜索索引与 RSS 里整个消失；
> 两个脚本现已按与 `lib/content.ts` 同一套规则递归（平铺 / 单篇文件夹 / 卡组）。
> 以后改内容目录结构时，**三个地方都要同步改**：`lib/content.ts`、`scripts/generate-search-index.mjs`、`scripts/generate-rss.mjs`。
...
普通文章卡片与卡组卡片都采用几何线条语言：文章卡片沿竖轴轻移、左侧出现强调色内线；卡组保留两层静态错位线框，hover 只改变边色与低对比度底色。两者都不再跟随指针或使用 3D 倾斜。
