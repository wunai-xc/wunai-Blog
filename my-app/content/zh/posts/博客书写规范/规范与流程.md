...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.12 | 2026-09-19 | 文章页左边缘浮动按钮列扩到四个（目录 → 搜索 → 标签 → 正文字号）：新增 `components/ArticleQuickLinks.tsx`（搜索/标签是链接不是面板）与标签总览页 `/zh/tags/`（此前只有单个标签页）；列位改用 `.post-layout:has(.toc-fab)` 统一移位，替掉原先的相邻兄弟选择器；2.4 节拆成「正文排版」与「浮动按钮列」两部分 |
...
13. **浮动导航**：左侧浮动按钮列（目录 + 面板、搜索、标签、正文字号 5–32px）、右侧可拖拽的阅读进度滑块（章节圆点）、回到顶部，均不占布局宽度。
...
│   ├── ArticleQuickLinks.tsx    # 左边缘浮动按钮：搜索 / 标签（链到 /<lang>/search/ 与 /<lang>/tags/）
...
自上而下四个：**目录**（`PostNav`，只在有标题时渲染）→ **搜索** → **标签**（后两个是链接，`ArticleQuickLinks`）→ **正文字号**（带面板，`ArticleFontSize`）。四个共用同一套外观与格距。

列位算法：`top = --chrome-top + 1px + N × --fab-step`。

| 情况 | 目录 | 搜索 | 标签 | 字号 |
|------|------|------|------|------|
| 文章有标题（有目录） | 第 1 格 | 第 2 格 | 第 3 格 | 第 4 格 |
| 文章没有标题（无目录） | — | 第 1 格 | 第 2 格 | 第 3 格 |

“目录不在场时其余三个各上提一格”由 **`.post-layout:has(.toc-fab)` 那一组规则**完成。
⚠️ **不要换回相邻兄弟选择器（`~`）**：那要求元素串联成链、每加一个按钮都要重算一次，
而且会被迫给它们包一层容器（`~` 只认同一父元素的兄弟）—— 包了就不再有效。
用 `:has()` 则是各自独立取位，增删一个按钮只改那一组。

三条硬约定：

1. **两个面板（目录、字号）的 `top` 跟着自己那个按钮走**（`44px` = 按钮高 + 10px 间隙），不能只写一个固定值。
2. **搜索与标签是链接（`<a>`），不是带面板的按钮**。它们指向已有的 `/<lang>/search/`（自带输入框 + Fuse 全文索引）与 `/<lang>/tags/`（标签云）；在这列里再抄一份搜索框/标签云只会多一份要同步维护的实现。
