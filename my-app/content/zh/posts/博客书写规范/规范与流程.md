...
1. **吸附写在 `<html>` 上，不另建 `height:100vh` 的内部滚动容器。**
   ...
   那样会把页脚永久挡住、读者根本够不到。
2. **吸附强度分两档，由 `components/HomeSnap.tsx` 判定。**
   `proximity` 温和、`mandatory` 最强，但 **mandatory 不能无条件用**：
   滚动容器一旦只能停在吸附点上，某屏高于视口时那一屏的**中段就再也滚不到**，
   读者会被卡在屏起点；最后一屏之后还会把页脚封在外面。
   所以：

   | 条件 | 强度 | 补充手段 |
   |------|------|----------|
   | 三屏都装得下视口 | `data-snap="strong"` → **mandatory** | 无需补充 |
   | 有屏超过视口（手机上「随便看看」是单列） | 保持 proximity | HomeSnap 的「向前拉一把」 |

   辅助吸附在滚动停稳（140ms）后介入：前方某屏起点在半屏以内、且**那一屏自己放得下**时就平滑滚过去。
   **只向前、不向后**——向后拉会把正在读长内容的读者拽回屏起点，
   也会在页脚处往上拽、使页脚够不到，正是当初不敢用 mandatory 的原因。
