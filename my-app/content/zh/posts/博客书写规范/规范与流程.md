...
13. **浮动导航**：左侧目录按钮 + 面板（其下还有「正文字号」按钮，5–32px，只管正文）、右侧可拖拽的阅读进度滑块（章节圆点）、回到顶部，均不占布局宽度。
...
│   ├── ArticleFontSize.tsx      # 正文字号浮动按钮：5–32px，只改正文（目录按钮下面一格；见 2.4）
...
- **小元件**（`.pagination`、`.term-item`、`.tag`、`.author-badge`、`.print-btn`、`.footer-link`、`.back-to-top`、`.toc-fab`、`.font-fab`）：它们数量多、叠上去显得吵；而且 20px 高的标签本就塞不下 5px 内缩的框，需要另调一套数值。已明确不要。
...
**2.4 正文排版：字号只有一个来源（`--article-fs`）**

文章页左边缘（顶栏下方）有两个浮动按钮：目录、正文字号（后者在目录按钮下面一格）。
字号按钮只改正文，怎么做到“只改正文”以及为什么这样写成了一整套约定：
...
- 位置靠 `--fab-step`（= 目录按钮高 34px + 8px 间隙，手机 38px）下移一格；
  用 `.toc-fab ~ .font-fab` 判断目录按钮在不在 —— **所以这两个元素必须是兄弟节点，不能包一层外层容器**。
